...
public class QueryDocAnnotator extends JCasAnnotator_ImplBase {
	...
	public void process(JCas aJCas) throws AnalysisEngineProcessException {
		...
		while (iter.hasNext()) {
			...
			String conceptText = cqc.getConceptQueryText();
			System.err.println("**" + arraylist.get(0).getText());
			System.err.println("****" + cqc.getConceptQueryText());

			// This one can return document type answer
			PubMedSearchServiceResponse.Result pubmedResult = null;
			PubMedSearchServiceResponse.Result conceptresult = null;
			try {
...
//			try {
//				conceptresult = goService.findPubMedCitations(conceptText, 0);
//			} catch (IOException e) {
//				// TODO Auto-generated catch block
//				e.printStackTrace();
//				System.out.println("PubMedResult exception!");
//			}
			...
			if (pubmedResult != null) {
				List<PubMedSearchServiceResponse.Document> docList = pubmedResult
						.getDocuments();
				// rank
				int rank = 0;
				...
				for (PubMedSearchServiceResponse.Document doc : docList) {
					...
					// docURI
					if (docSet.add(docID)) {
						String uri = "http://www.ncbi.nlm.nih.gov/pubmed/"
								+ docID;
						// Title
						...
						// new a document
						Document document = TypeFactory.createDocument(aJCas,
								uri, 0.0, abs, rank, queryText, "",
								new ArrayList<CandidateAnswerVariant>(), title,
								docID);
			...
			if (conceptresult != null) {
				System.out.println("concept retrieval....");
				List<PubMedSearchServiceResponse.Document> docList = pubmedResult
						.getDocuments();
				int rank = 0;
				// iterate through the whole documents
